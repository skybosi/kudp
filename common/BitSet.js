...
(function (g, f) {
  ...
  /**
   * grow a bitset while all bit are be use
   * @returns {BitSet} an copy orgin bitset data
   */
  BitSet.prototype.grow = function () {
    this.MAX_BIT *= 2;
    let newBS = new BitSet(this.dehydrate());
    this.arr = newBS.arr;
  };
   ...
   */
  BitSet.prototype.clone = function (dehydrate) {
    return new BitSet(dehydrate || this.dehydrate());
  };
  ...
  BitSet.prototype.ffs = function (_startWord) {
    ...
    (fs > this.MAX_BIT) ? this.grow() : null;
  ...
  BitSet.prototype.ffz = function (_startWord) {
    ...
    (fz > this.MAX_BIT) ? this.grow() : null;
