...
(function (root, factory) {
    ...
    class Sender {
        ...
        kudp_send(fdtn, buf, len, flag) {
            ...
            if (!utils.isNull(task_ctx)) {
                ...
                if (nbuf < task_ctx.snd_buf_wnd) {
                    var { seq, pack_ctx } = this._snd_data_seq(task_ctx, buf, len, flag)
        ...
        __rcv_notify_ack(fdtn) {
            ...
            for (var seq in ack_ctx.list) {
                ...
                var { cf, tn, connid, rcv_ts, rmt_wnd, mtype } = ackpack_ctx
                var ctx = { rcv_ts, rwnd }
                var ack_type = ACK_TYPE[mtype]
                var ackpack = new Package(ack_type, 0, cf, tn, seq, connid, ctx)
                this.kudp_output(address, port, ackpack.buffer)
                ack_ctx.cnt--
                delete ack_ctx.list[seq]
            }
            logger.info("__rcv_notify_ack", fdtn)
        }
    ...
    class Recver {
         ...
         * @param {*} ackpack 
         */
        _rcv_task_ack_push(fdtn, seq, ackpack_ctx) {
            var task_ctx = this._rcv_task_ctx(fdtn)
            var ack_ctx = this.conn.get_task_ctx(fdtn, ACK_NAME)
            if (task_ctx && !ack_ctx) {
            ...
            ack_ctx.cnt++
            logger.info("_rcv_task_ack_push", ack_ctx)
        ...
        kudp_input(address, port, buffer) {
            ...
            switch (mtype) {
                ...
                case WSASK: // 计算wnd, 生成WSANS返回
                    ...
                    this._rcv_task_ack_push(fdtn, seq, ackpack_ctx)
                    break;
                case PINGS: // 计算wnd
                    var ackpack_ctx = { cf, tn, connid, rcv_ts, rmt_wnd, mtype, ts }
                    this._rcv_task_ack_push(fdtn, seq, ackpack_ctx)
