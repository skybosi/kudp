...
(function (g, f) {
    ...
    class Recver {
        ...
        checkQ(node) {
            ...
            if (this.isOver(node)) {
                ...
                }, { seq: node['seq'] }).start(ACK_TIMEOUT * 1000);
            }
        }
        ...
        checkRcvBuf(node, content) {
            ...
            if (len + offset < max) {
                ...
                node['P']++;
                if (this.isOver(node)) {
                    LOG.info("checkRcvBuf:", node['BF']._woffset, node['BF'].slice(0, node['BF']._woffset).toString())
    ...
    class Sender {
        ...
        retry(seq, mtype, ip, port, pack, ctx) {
            LOG.warn('retry: ', seq, mtype, ip, port, this.timeout)
            ...
            if (this.timers[Sender.prefix + seq].repeat < this.repeat) {
                ...
                this.onErrs(seq, mtype, peer, this);
            }
        }
