...
(function (g, f) {
    ...
    class Recver {
        ...
        addSeqQueue(node, seq, content, lo) {
            ...
            this.addSeqTimer(mtype, node['seq'], node);
        }
        ...
        // 释放接收队列中的已经确认的seq
        freeSeq(isn, node) {
            let Q = node['Q'], M = node['M'];
            let ctx = Q.ctx, lastseq = ctx.P;
            let [mtype, ip, port, stime] = [ctx.mtype, ctx.peer.address, ctx.peer.port, +new Date];
            if (lastseq) {
                // LOG.info("onEcho seq:", isn, lastseq);
    ...
    class SendBuffer {
        ...
        free(cursor, lo, cb) {
            if (cursor < this._cursor)
                return;
            for (let i = this._cursor; i <= cursor; i++) {
        ...
        free(seq, mtype, peerInfo) {
            ...
            if (mtype > AMULTI) {
                ...
                if (curnode && curnode.data && curnode.data['Q'] instanceof SendBuffer && !curnode.data['Q'].empty()) {
                    ...
                    // 删除累计确认过程中的所有seq
                    curnode.data['Q'].free(seq, null, (ctx, node) => {
                        this.seqer.del(node[1][1], isn);
                        node && this.calcRto(node[1][2]);
                    })
                    // LOG.info("free ack seq ok:", seq);
                    // 尝试释放重试定时器
                    let isnTimer = this.timers[this.timerPrefix + isn]
                    if (!isnTimer) {
        ...
        addSeqTimer(seq, mtype, ip, port, pack, fd) {
            ...
            switch (mtype) {
                ...
                case BEGIN: case BDD:
                    ...
                    this.buffer[seq] = Q;
                    this.queue.insert({ seq: seq, L: seq + section, Q: Q, fd: fd }, { done: 0, stat: 0 });
    ...
    class kudp extends UdpBase {
        ...
        _onRdone(seq, mtype, peer, stat, ctx) {
            ...
            return this.sQueue.write(null, peer.address, peer.port, mtype, seq);
        }
