...
(function (root, factory) {
    ...
    class Sender {
         ...
         */
        _snd_task_ctx(fd, tn) {
            if (this.snd_task[fd] && this.snd_task[fd][tn]) {
                return this.snd_task[fd][tn]
            }
            return null
        }
         ...
         * @param {*} tn task number 任务
         */
        _shrink_snd_buf(fd, tn) {
            var task_ctx = this._snd_task_ctx(fd, tn)
            // 当前的发送窗口
        ...
        create(fd, swnd, rwnd) {
            ...
            this.snd_task[fd] = this.snd_task[fd] || {}
            this.snd_task[fd][tn] = {
         ...
         * @param {*} ackpack 
         */
        __rcv_notify_snd(fdtn, fd, tn, ts, rmt_wnd, ackpack) {
            var task_ctx = this._snd_task_ctx(fd, tn)
            if (task_ctx && !utils.isNull(rmt_wnd)) {
                task_ctx.rmt_wnd = rmt_wnd
            }
            console.log("__rcv_notify_snd", fdtn, tn, ts, ackpack)
        }
         ...
         * @param {*} delta 
         */
        __update_snd_rto(fd, tn, ts, delta) {
            var task_ctx = this._snd_task_ctx(fd, tn)
         ...
         * @param {*} rmt_wnd 
         */
        __update_snd_rmt_wnd(fd, tn, rmt_wnd) {
            var task_ctx = this._snd_task_ctx(fd, tn)
            if (task_ctx) {
                task_ctx.rmt_wnd = rmt_wnd
         ...
         * @param {*} mtype 
         */
        __rcv_notify_ctrl(fd, tn, pcf, mtype) {
            var task_ctx = this._snd_task_ctx(fd, tn)
            switch (mtype) {
                case RESET:  // 重置逻辑
    ...
    class Recver {
         ...
         * @returns 
         */
        _rcv_task_rwnd(recv_id, tn) {
            var task_ctx = this._rcv_task_ctx(recv_id, tn) || {}
         ...
         * @param {*} recv_id 
         */
        _rcv_task_ctx(recv_id, tn) {
            if (this.rcv_task[recv_id] && this.rcv_task[recv_id][tn]) {
                return this.rcv_task[recv_id][tn]
            }
            return null
        }
         ...
         * @param {*} option 
         */
        _update_rcv_task_ctx(recv_id, tn, option) {
            var task_ctx = this._rcv_task_ctx(recv_id, tn) || {}
            for (var key in option) {
                task_ctx[key] = option[key]
            }
        }
        ...
        _rcv_task_init(cid, address, port, tn, mtype, seq, rwnd) {
            var recv_id = [address, port].join(SEP)
            if (cid && cid != "0x") {
                recv_id = cid
            }
            // 如果需要，初始化isn
            ...
            if (this.rcv_task[recv_id] && this.rcv_task[recv_id][tn]) {
                if (isn && !this.rcv_task[recv_id][tn]["isn"]) {
                    this.rcv_task[recv_id][tn]["isn"] = isn
                }
                return recv_id
            }
            ...
            this.rcv_task[recv_id] = this.rcv_task[recv_id] || {}
            this.rcv_task[recv_id][tn] = this.rcv_task[recv_id][tn] || {
         ...
         * @param {*} ackpack 
         */
        _rcv_task_ack_push(recv_id, tn, ts, rmt_wnd, ackpack) {
            var task_ctx = this._rcv_task_ctx(recv_id, tn) || {}
            var { cid, fd, fdtn, address, port } = task_ctx
            this.sender.__rcv_notify_snd(fdtn, fd, tn, ts, rmt_wnd, ackpack)
        }
        ...
        kudp_input(address, port, buffer) {
            ...
            var { fd, recv_id } = this._rcv_task_init(cid, address, port, tn, mtype, seq)
            // 通知sender 计算更新rto
            if (fd && tn && ts) {
                this.sender.__update_snd_rto(fd, tn, ts, ctx.delta || 0)
            }
            console.log(kudpack.toHex(), body, ctx)
            console.log(header.toHex())
            // 计算本端接收窗口
            var rwnd = this._rcv_task_rwnd(recv_id, tn)
            // 从数据包获取对端的接收窗口rwnd
            ...
            switch (mtype) {
                ...
                case WSANS: // 窗口的大小回复通知, 更新 rmt_wnd
                    this.sender.__update_snd_rmt_wnd(rmt_wnd)
                    break;
                ...
                case WSASK: // 计算wnd, 生成WSANS返回
                    ...
                    this._rcv_task_ack_push(recv_id, tn, ts, rmt_wnd, ackpack)
                    break;
                case PINGS: // TODO 是否需要响应
                    var ack_ctx = { rcv_ts, rwnd }
                    var ackpack = new Package(DTACK, 0, cf, tn, seq, connid, ack_ctx)
                    this._rcv_task_ack_push(recv_id, tn, ts, rmt_wnd, ackpack)
                    break;
                case BEGIN: // payload 写入rcv_buf，检测rcv_nxt 进而写入rcv_que
                case DONED: // payload 写入rcv_buf，检测rcv_nxt 进而写入rcv_que，更新fdtn 的state 为 SND_STATE_DONE
                    break;
                case DOING: // payload 写入rcv_buf，检测rcv_nxt 进而写入rcv_que
                    var ack_ctx = { rcv_ts, rwnd }
                    var ackpack = new Package(DTACK, 0, cf, tn, seq, connid, ack_ctx)
                    this._rcv_task_ack_push(recv_id, tn, ts, rmt_wnd, ackpack)
                case BDODO: // 计算ACK NACK NDASK
                    var ack_ctx = { rcv_ts, rwnd }
                    var ackpack = new Package(DTACK, 0, cf, tn, seq, connid, ack_ctx)
                    this._rcv_task_ack_push(recv_id, tn, ts, rmt_wnd, ackpack)
                    break;
