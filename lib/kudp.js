...
(function (g, f) {
    ...
    class Recver {
        ...
        push(mtype, seq, peerInfo, payload, rangesize, version) {
            ...
            // 添加seq定时器
            this.addSeqTimer(mtype, seq, peerInfo.address, peerInfo.port);
        }
        ...
        addSeqTimer(mtype, seq, ip, port) {
            ...
            switch (mtype) {
                case BEGIN:
                    ...
                    this.timers[Recver.prefix + "isn" + SEP + isn] = isnTimer;
                ...
                case DOING: case DONED:
                    ...
                    if (curnode && curnode.data) {
                        ...
                        isnTimer = this.timers[Recver.prefix + "isn" + SEP + isn];
                        Q = isnTimer.args[1]
                        Q.insert(seq, [mtype, ip, port, +new Date]);
        ...
        // 释放接收队列中的已经确认的seq
        freeSeq(isn, Q) {
            let lastseq = null, lastother = null;
            let delseq = []
            // 发送待确认队列中的残余数据
                ...
                if (curnode && curnode.data && curnode.data['Q'] instanceof seqTree && 0 !== curnode.data['Q'].size) {
                    ...
                    if (isnTimer) {
                        ...
                        if (0 === curnode.data['Q'].size && 1 === (isnTimer.args || {}).done) {
                            isnTimer.stop() && delete this.timers[Sender.prefix + "isn" + SEP + isn];
                        }
                    } else {
