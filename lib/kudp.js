...
(function (root, factory) {
    ...
    class Sender {
        ...
        __rcv_notify_ack(fdtn, pcf = 0) {
            ...
            for (var seq in ack_ctx.list) {
                var { ackpack_ctx, rwnd } = ack_ctx.list[seq]
         ...
         * @param {*} mtype 
         */
        __update_snd_rmt_wnd(fdtn, tn, rmt_wnd, mtype) {
            var task_ctx = this._snd_task_ctx(fdtn)
            if (task_ctx) {
    ...
    class Recver {
        ...
        _rcv_task_dack_push(fdtn, seq, mtype, ackpack_ctx) {
            ...
            ack_ctx.list[seq] = { ackpack_ctx, rwnd }
            ack_ctx.cnt++
            // 计算una, sack, nack
            this.sender.__rcv_notify_ack(fdtn)
        }
        ...
        kudp_input(address, port, buffer) {
            ...
            switch (mtype) {
                ...
                case DTACK: // 更新rtt rmt_wnd
                    this.sender.__update_snd_rmt_wnd(fdtn, tn, rmt_wnd, mtype)
                    break;
