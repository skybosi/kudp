...
(function (g, f) {
    ...
    // 重传机制超时时间
    const ACK_TIMEOUT = 3000 // RFC1122 RFC6298  RFC2988
    // 局域网最大数据包大小
    ...
    class Package {
        ...
        static pack(header, seq, payload, factor) {
            ...
            // }
            // /** 写入数据包内容 */
            // msg.write(payload.toString(), HEADER_LEN)    // 8 消息内容
            ...
            /** 写入数据包内容 */
            msg2.writeBuffer(payload)           // 8 消息内容
    ...
    class Recver {
        ...
        onMessage(res) {
            ...
            if (mtype < ABROAD) {
                this.push(mtype, seq, peerInfo, payloadbuffer, rangesize, version);
            } else {
        ...
        begin(isn, peerInfo, payload, rangesize, version) {
            ...
            node._ = curnode;
            // 用优先队列实现接收的数据包排序
            this.addSeqQueue(node, isn, [BEGIN, payload]);
            // 检测所有暂存区中已有的seq
            for (var seq in this.staging) {
                let seq_data = this.staging[seq];
                let mtype = seq_data[0], seq1 = seq_data[1],
                    peer2 = seq_data[2], payload3 = seq_data[3];
                // 记录最大seq
                if (DONED === mtype)
                    curnode.data['M'] = +seq1 + 1;
                if (peer2.address === peerInfo.address && peer2.port === peerInfo.port &&  // 必须是同一个ip:port
                    seq1 >= node.seq && seq1 < node.L) {
                    this.addSeqQueue(node, seq1, [mtype, payload3]);
         ...
         */
        checkRcvBuf(node, content) {
            let mtype = content[0], payload = content[1];
            let len = payload.length // Buffer.byteLength(payload, 'utf8')
            ...
            if (curnode) {
                ...
                if (curnode.data) {
                    ...
                    if (curnode.data['P'] > seq) {
                        ...
                        this.addSeqQueue(curnode.data, seq, [mtype, payload]);
        ...
        // 添加收到seq 到 Q
        addSeqQueue(node, seq, content) {
            if (seq >= node['P']) {
                node['Q'].push(content, seq);
            } else {
        ...
        addSeqTimer(seq) {
            ...
            if (!seqTimer) {
                ...
                this.stat.incr(Recver.prefix + 'timer_count');
                // LOG.info("recver_timer_addSeqTimer: ", this.stat.get(Recver.prefix + 'timer_count'));
            } else {
                seqTimer.restart(ACK_TIMEOUT * 2);
                this.stat.incr(Recver.prefix + 'restart_count');
                // LOG.info("recver_timer_addSeqTimer: ", this.stat.get(Recver.prefix + 'restart_count'));
            }
        }
        ...
        freeSeq(args) {
            ...
            if (1 === (seqTimer.args || {}).timeout) {
                ...
                this.stat.decr(Recver.prefix + 'timer_count');
                return;
            }
            // 从queue中定位对应的isn
            let curnode = this.location(seq);
    ...
    class Sender {
        ...
        encode(fd, mtype, payload, max_size) {
            ...
            let pkg = { seq: seq, size: size, type: mtype, payload: buff, factor: FACTOR }
            let pack = new Package(mtype, 0, 0, 0, pkg);
            fstat.ip && this.addSeqQueue(mtype, seq, pkg.factor, fstat.ip, fstat.port, pack);
        ...
        addSeqQueue(mtype, seq, factor, ip, port, pack) {
            switch (mtype) {
                case BEGIN:
                    let L = seq + factor * BASE_SECTION;
                    let Q = new seqTree(Sender.prefix, (a, b) => a - b);
                    Q.insert(seq, [mtype, seq, ip, port, pack])
                    this.queue.insert({ seq: seq, L: L, Q: Q }, { done: 0, stat: 0 });
                    return
                case BDD: case BROAD: case MULTI:
                    // let Q = new seqTree(Sender.prefix, (a, b) => a.seq - b.seq);
                    // Q.insert(seq, [mtype, seq, ip, port, pack])
                    // this.queue.insert({ seq: seq, Q: Q }, { done: 0, stat: 0 });
                    return
                case DOING: case DONED:
                    break;
                default:
            ...
            if (curnode && curnode.data && curnode.data['Q'] instanceof seqTree) {
                curnode.data['Q'].insert(seq, [mtype, seq, ip, port, pack]);
            } else {
        ...
        retry(seq, mtype, ip, port, pack, ctx) {
            ...
            if (curnode && curnode.data && curnode.data['Q'] instanceof seqTree) {
                curnode.data['Q'].each((data, other) => {
                    ...
                    let [mtype, _, ip, port, pack] = other;
        ...
        free(seq, mtype, peerInfo) {
            if (mtype > AMULTI) {
                let curnode = this.location(seq);
                if (curnode && curnode.data && curnode.data['Q'] instanceof seqTree) {
                    curnode.data['Q'].remove(seq);
                    // 尝试释放重试定时器
                    let isn = curnode.data['seq']
                    let isnTimer = this.timers[Sender.prefix + "isn" + SEP + isn]
                    ...
                    if (0 === curnode.data['Q'].size && 1 === (isnTimer.args || {}).done) {
                        delete this.timers[Sender.prefix + "isn" + SEP + isn];
                    }
                } else {
                    LOG.error("addSeqQueue empty");
                }
        ...
        addSeqTimer(seq, type, ip, port, pack) {
            ...
            isnTimer = this.timers[Sender.prefix + "isn" + SEP + isn];
            if (isnTimer) {
                let status = isnTimer.status;
                if (isnTimer.repeat >= 10) {
                    let peer = { address: ip, port: port };
                    this.onErrs(seq, type, peer, this);
                    isnTimer.stop();
                } else if ('stopped' === status && 0 !== curnode.data['Q'].size) {
                    isnTimer.restart(ACK_TIMEOUT);
                }
            } else {
                isnTimer = new timer({
                    onend: (args) => { this.retry(...args); },
                    onstop: (...args) => {
                        LOG.info("addSeqTimer", ...args);
                    }
                }, [isn, type, ip, port, pack, this]);
                isnTimer.setArgs({ done: 0, timeout: 0 });
                this.timers[Sender.prefix + "isn" + SEP + isn] = isnTimer;
                isnTimer.start(ACK_TIMEOUT);
            }
        }
    }
    ...
    class kudp extends UdpBase {
        ...
        // 发送传输异常回调
        _onWerr(seq, mtype, peer, ctx) {
            this.onWerr(seq, mtype, peer, ctx);
        }
