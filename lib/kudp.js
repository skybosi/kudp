...
(function (g, f) {
        ...
        get(isn) {
            ...
            if (isn > -1 && this.data[isn]['cursor']) {
                ...
                if (this.data[isn]['cursor'] >= this.data[isn]['last']) {
                    // LOG.info("isn, seq, last", isn, seq, this.data[isn]['last'], lo)
                }
    ...
    class Package {
        ...
        static unpack(buffer) {
            ...
            // pkg.payload = msg.readBuffer(HEADER_LEN);         // 8 消息内容
    ...
    class RecvBuffer {
        constructor(cap, isn, cb, ctx) {
            this._cap = cap;    // 最大容量
            this._isn = isn || 0;
            this._cb = cb;
            this._ctx = ctx;
            this._buf = Buffer.alloc(KUDP_RCVBUF);
            this.init();
        }
        ...
        push(pos, lo, data) {
            // TODO stage 过多清理算法
            // if (this._stage.length >= this._cap) {
            //     LOG.error("RecvBuffer stage over", this);
            //     return 4
            // }
            pos -= this._isn;
            if (pos >= this._cap)
        ...
        /**
         * 将收到的有序的数据写入接收buf，合适的时机回调上报业务层
         *  - 接收缓冲区已满
         *  - 接收内容结束
         * @param {*} content_list
         */
        buffer(content_list) {
            if (!content_list || 0 === content_list.length)
                return
            for (let i = 0, clen = content_list.length; i < clen; i++) {
                let content = content_list[i]
                let mtype = content[0], seq = content[1], payload = content[2];
                this._ctx.P = seq, this._ctx.mtype = mtype;
                let len = payload.length, offset = this._buf._woffset, max = this._buf.length;
                if (len + offset < max) {
                    this._buf.writeBuffer(payload);
                } else { //  len + offset >= max
                    if (typeof this._cb === 'function') {
                        this._cb(this._ctx, this._buf.readBuffer(0, this._buf._woffset))
                    };
                    this._buf.flush();
                    this._buf.writeBuffer(payload);
                }
        ...
        static testRecvBuffer() {
            let cap = 6
            let rb = new RecvBuffer(cap, 0, (i, data) => {
            ...
            while (i < max) {
                let dt = data[i]
                rb.push(dt, 0, [0, 0, Buffer.fromString("data:" + dt + " ")]);
                i++;
            }
            rb.flush();
            rb.clear();
            ...
            while (i < data.length) {
                ...
                let dt = data[i] % cap
                rb.push(dt, lo, [i, i, Buffer.fromString("data:" + lo + ":" + data[i] + " ")]);
                i++;
            }
            rb.flush();
            LOG.info(data);
    ...
    class Recver {
        ...
        begin(isn, peerInfo, payload, rangesize, version, lo) {
            let node = {
                ...
                T: BEGIN,                  // node.T 数据包类型
                V: version,                // node.V 数据包类型及协议版本
                t: +new Date
            }
            // other 标记该queue的节点是否已经被操作完成，此处表示该节点对应的数据块是否已经接收完
            let other = { done: 0, stat: 0 }
            let status = this.queue.insert(node, other);
            if (false == status) {
                LOG.warn("begin Repeat:", isn)
                return;
            }
            let ctx = { peer: peerInfo, seq: isn, P: isn }
            // node.Q 记录已经收到的数据，保证进入乱序出来有序
            node.Q = new RecvBuffer(rangesize, isn, this.checkRcvBuf.bind(this), ctx);
            node.other = other;
            LOG.info("begin:", isn, node);
            // 用优先队列实现接收的数据包排序
            this.addSeqQueue(node, isn, [BEGIN, isn, payload], lo);
            // 检测所有暂存区中已有的seq
            for (var seq in this.staging) {
                ...
                if (peer2.address === peerInfo.address && peer2.port === peerInfo.port &&
                    seq1 >= node.seq && seq1 < node.L) {
                    node['lo'] = lo4;
                    this.addSeqQueue(node, seq1, [mtype, seq1, payload3], lo4);
                    this.stage_size--;
        ...
        checkQ(node) {
            ...
            LOG.info("onEcho checkQ isn, seq, max:", node['seq'], node['M'], node['M']);
            this.onEcho(node['peer'].address, node['peer'].port, DONED, node['M'], +new Date);
         ...
         */
        checkRcvBuf(ctx, buff) {
            this.onUp(ctx['seq'], ctx['mtype'], ctx['P'], ctx['peer'], buff);
        }
        ...
        stage(mtype, seq, peerInfo, payload, lo) {
            ...
            if (curnode) {
                ...
                if (curnode.data) {
                    ...
                    if (DONED === mtype) {
                        curnode.data['M'] = +seq;
                        LOG.info("stage max:", curnode.data['seq'], seq, lo)
                    }
                    if (seq > curnode.data['M'])
                        return
                    // TODO 判断数据包接收结束的最佳方法
                    this.addSeqQueue(curnode.data, seq, [mtype, seq, payload], lo);
                    if (curnode.data['M'] <= curnode.data['Q'].ctx['P']) {
                        this.checkQ(curnode.data)
                    }
                }
            } else {
                // 未找到BEGIN的数据包, 为了避免恶意攻击，限制最大缓存max_stage个无 BEGIN 数据包
                if (this.stage_size++ <= this.max_stage)
                    this.staging[seq] = [mtype, seq, peerInfo, payload, lo];
            }
        }
        ...
        addSeqQueue(node, seq, content, lo) {
            ...
            // 添加seq定时器
            this.addSeqTimer(content[0], node['seq'], node);
            if (node['Q'].size > Recver.UPCD_SIZE) {
                this.freeSeq(node);
            }
        }
         ...
         */
        addSeqTimer(mtype, isn, node) {
            switch (mtype) {
                case BEGIN:
                    if (!this.timers[this.timerPrefix + isn]) {
                        let isnTimer = new timer({
        ...
        // 释放接收队列中的已经确认的seq
        freeSeq(node) {
            let isn = node['seq'], Q = node['Q'], M = node['M'], ctx = Q.ctx, lastseq = ctx.P;
            let [mtype, ip, port, stime] = [ctx.mtype, ctx.peer.address, ctx.peer.port, +new Date];
            if (lastseq) {
                LOG.info("onEcho isn, seq, max:", isn, lastseq, M);
                this.onEcho(ip, port, mtype, lastseq, stime);
                let isnTimer = this.timers[this.timerPrefix + isn];
                if (isnTimer && "stopped" == isnTimer.status && M > lastseq + 1)
        ...
        static testRecver() {
            ...
            let peerInfo = { address: "127.0.0.1", port: DEFAULT_PORT };
            // 新建一个测试任务表
    ...
    class Connector {
        ...
        open(option) {
            ...
            let fd = this.coonnset.setInfo(option);
            return fd;
        }
        // 关闭一次传输, 释放对应的fd
        close(fd) {
    ...
    class SendBuffer {
        ...
        init(isn) {
            ...
            this._cursor = isn || 0;    // 当前连续的已被读取起始下标
            this._los = { 0: 1 };  // 接收的轮次计划
        }
        ...
        push(pos, lo, data) {
            ...
            pos -= this._isn;
            // TODO 是否考虑lo
            let index = pos + SEP + this._lo + SEP + this._isn;
            if (!this._pool[index]) {
                this._pool[index] = [lo, data];
                this._size++;
            }
            return 0;
        }

        // 释放 this._cursor -> cursor 的数据, TODO 是否考虑lo
        free(cursor, lo, cb) {
            if (cursor < this._cursor)
                return;

            for (let i = this._cursor, delta = cursor - this._cursor; i <= cursor; i++) {
                let index = (i - this._isn) + SEP + this._lo + SEP + this._isn;
                if (this._pool[index]) {
                    if (typeof cb === 'function')
                        cb(this._ctx, this._pool[index], delta);
                    delete this._pool[index];
                    this._size--;
                };
            }
            (0 >= this._size) ? this._cursor = this._isn : this._cursor = cursor + 1;
        }

        del(pos, lo, cb) {
            let index = (pos - this._isn) + SEP + this._lo + SEP + this._isn;
            if (typeof cb === 'function' && this._pool[index]) {
                if (typeof cb === 'function')
                    cb(this._ctx, this._pool[index]);
                delete this._pool[index];
                this._size--;
            };
            (0 >= this._size) ? this._cursor = this._isn : this._cursor = cursor + 1;
    ...
    class Sender {
        ...
        constructor(options) {
            ...
            this.buffer = {};               // 发送缓冲区，与this.queue中的node一一对应，每个连接一个发送缓冲区 TODO 单个连接独立缓冲区是否可靠
        ...
        checkBufferFull(isn) {
            ...
            let curnode = this.buffer[isn];
            if (curnode && curnode instanceof SendBuffer) {
                return curnode.full();
            } else {
                LOG.error("checkBuffer error:", isn);
            }
            return false;
        }
        ...
        retry(isn, ip, port) {
            let curnode = this.queue.find({ seq: isn });
            if (curnode && curnode.data && curnode.data['Q'] instanceof SendBuffer && !curnode.data['Q'].empty()) {
                // 防止死循环，导致无限重试
                let isnTimer = this.timers[this.timerPrefix + isn];
                // 当前已确认的seq 与 初始isn 相同,说明在指定的时间内未收到任何ack，很大可能存在网络不通
                if (curnode.data['P'] <= curnode.data['seq']) {
                    // isnTimer.stop();
                    this.onErrs(isn, { address: ip, port: port }, this);
                    return
                } else {
                    // 已收到确认，但是出现较大延迟
                ...
                // 发送待确认队列中的残余数据
                curnode.data['Q'].each((ctx, data) => {
                    // LOG.info("retry left seq:", ctx, data);
                    let ip = ctx.ip, port = ctx.port;
                    let [mtype, pack, _] = data[1];
                    data[1][3] = +new Date;
        ...
        free(seq, mtype, peerInfo) {
            ...
            if (mtype > AMULTI) {
                ...
                if (curnode && curnode.data && curnode.data['Q'] instanceof SendBuffer && !curnode.data['Q'].empty()) {
                    ...
                    // 删除累计确认过程中的所有seq
                    curnode.data['Q'].free(seq, null, (ctx, node, delta) => {
                        let seq = node[1][1].seq, tm = node[1][2];
                        this.seqer.del(seq, isn);
                        curnode.data['P'] = seq;
                        node && this.calcRto(tm, delta);
                    })
                    LOG.info("free ack seq ok isn, seq:", isn, seq);
                } else {
                    LOG.debug("free seq empty Q:", curnode);
                }
                // 队列为空，且已经收到结束数据包，释放定时器
                if (curnode.data['Q'].empty()) {
                    // 尝试释放重试定时器
                    let isnTimer = this.timers[this.timerPrefix + isn]
                    if (!isnTimer) {
                        LOG.error("free isnTimer", seq);
                        return
                    }
                    if (1 === (isnTimer.args || {}).done) {
                        isnTimer.stop() && delete this.timers[this.timerPrefix + isn];
                        this.conn.clearInfo(curnode.data['fd']);
                        delete this.buffer[isn];
                        // 尝试释放isn
                        ...
                        // 发送缓冲区释放空间，通知业务层继续发送
                        let chunked = this.buffer[isn].chunked;
                        chunked.cb(chunked.ctx);
                    }
                }
            } else {
                // TODO
        ...
        // 添加seq 重试定时器
        addSeqTimer(seq, mtype, ip, port, pack, fd) {
            let Q = null;
            switch (mtype) {
                case DOING: case DONED:
                    break;
                case BEGIN: case BDD:
                    // BDD小型数据包section为1，否则为 factor 的整数倍
                    let section = (mtype === BEGIN) ? (pack.factor * BASE_SECTION) : 1;
                    // 创建发送缓冲器
                    Q = new SendBuffer(section, seq, null, { ip: ip, port: port, seq: seq });
                    Q.push(seq, 0, [mtype, pack, +new Date]);
                    this.buffer[seq] = Q;
                    this.queue.insert({ seq: seq, P: seq, L: seq + section, Q: Q, fd: fd }, { done: 0, stat: 0 });
                    this.addSendTimer(seq, ip, port).start(this.rto); // 创建发送定时器
                    return
            ...
            let curnode = this.location(seq);
            if (curnode && curnode.data && curnode.data['Q'] instanceof SendBuffer) {
                Q = curnode.data['Q'];
                // TODO 是否存在 ip, port 错乱
                Q.push(seq, 0, [mtype, pack, +new Date]);
                if (mtype === DONED) {
        ...
        chunked(cb, ctx) {
            let seq = ctx.seq;
            if (this.buffer[seq]) {
                this.buffer[seq].chunked = { cb: cb, ctx: ctx }
            } else {
                LOG.error("chunked:", ctx);
            }
        }
    }
    ...
    class kudp extends UdpBase {
        ...
        // 新建一次新的传输过程，分配一个唯一的fd
        open(ip, port, flag) {
            return this.sQueue.open({ ip: ip, port: port, flag: flag });
        }
        ...
        write(fd, ip, port, payload, flag) {
            let ret = this.sQueue.write(fd, ip, port, flag || BEGIN, payload);
