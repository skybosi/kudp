...
(function (root, factory) {
    ...
    const HeaderType = kupack.HeaderType      // 类型
    const rHeaderType = kupack.rHeaderType    // 反类型
    ...
    const MIN_TASK_NUM = kupack.MIN_TASK_NUM
    ...
    // kudp error
    const EKUDPOK = 0
    const ESINVALID_FDTN = 1 // 无效fdtn，获取不到_snd_task_ctx
    const ESSNDBUF_OVER = 3 // 空的无效acklist
    const ESEMPTY_ACKQ = 5 // 发送缓冲区溢出
    const ERINVALID_FDTN = 2 // 无效fdtn，获取不到_rcv_task_ctx
    const ERINVALID_PACK = 4 // 解析接收的数据包异常
    const ERINVALID_PACK2 = 6 // 数据包校验失败
    const ERINVALID_MTYPE = 8 // 无效的数据包类型
    ...
    const SND_ACK_PCF_DEFAULT = 1  // 默认回复ack的pcf, 即：ack + una
    const SND_DEADLINK = 20        // 重试最大次数后，认为是连接通道异常
    ...
    class Connector {
         ...
         * @returns 
         */
        get_task_ctx(oid, source) {
            if (this.task[oid] && this.task[oid][source]) {
                return this.task[oid][source]
            }
            return null
        }
        ...
        newTask(oid, source, ctx) {
            if (null != this.get_task_ctx(oid, source)) {
                for (var key in ctx) {
    ...
    class Sender {
        ...
        _check_snd_task_ctx(fdtn) {
            return this.conn.get_task_ctx(fdtn, SND_NAME)
        }
         ...
         */
        _snd_task_ctx(fdtn) {
            var task_ctx = this._check_snd_task_ctx(fdtn)
            if (null == task_ctx) {
                this.error(fdtn, ESINVALID_FDTN, 'invalid snd fdtn')
            }
            return task_ctx
        }
         ...
         * @param {*} mtype 
         */
        _snd_0wnd_ask(fdtn, tn, mtype) {
            var task_ctx = this._snd_task_ctx(fdtn)
            var { cf, connid, address, port } = task_ctx
            var ctx = { tn }
            var pcf = tn < MIN_TASK_NUM ? 0 : 1
            var ackpack = new Package(WSASK, pcf, cf, tn, 0, connid, ctx)
         ...
         * @param {*} flag
         */
        _snd_data_seq(task_ctx, buf, len, flag) {
            var { tn, cf, sndcnt, snd_state, connid } = task_ctx
            var pack_ctx = null
            var seq = this.seqer.cursor(tn)
            var mtype = null
            // 判断当前数据包是否是该task的第一个数据包
            if (sndcnt == 0 && snd_state == SND_STATE_INIT) {
                // 判断当前数据包是否小于1个MSS
                mtype = (len < MSS) ? BDODO : BEGIN
                pack_ctx = { mtype, flag, cf, tn, seq, connid, buf }
            } else {
                // 判断当前数据包是否小于1个MSS
                mtype = (len < MSS) ? DONED : DOING
                pack_ctx = { mtype, flag, cf, tn, seq, connid, buf }
            }
            // 更新snd_nxt
            if (mtype != BDODO) {
        ...
        _add_snd_que(task_ctx, seq, pack_ctx) {
            if (task_ctx.nsnd_que >= task_ctx.snd_que_siz) {
                this.error(task_ctx.fdtn, ESSNDBUF_OVER, 'snd queue overflow')
                return -1
            }
            var { fdtn } = task_ctx
            ...
            if (task_ctx.snd_state <= SND_STATE_STASH) {
                ...
                if (!task_ctx.snd_timer) {
                    task_ctx.snd_timer = new timer({
                        onend: (args) => { this._update_snd_buf(...args) },
                        onstop: (args) => { logger.info("timer onstop:", ...args) }
                    }, [fdtn])
        ...
        _update_snd_buf(fdtn) {
            ...
            for (var seq in task_ctx.snd_buf) {
                ...
                if (needsend) {
                    ...
                    console.log('sending', seq, buf)
                    if (segment.xmit >= task_ctx.dead_link) {
                        task_ctx.snd_state = SND_STATE_TIMEOUT
                        logger.error(task_ctx.snd_state)
                    }
                }
                logger.info("send", seq, sndcnt)
            }
            // 重试超时，触发重试定时器关闭，认为连接异常
            if (task_ctx.snd_state == SND_STATE_TIMEOUT) {
                task_ctx.snd_timer.close()
            } else {
                task_ctx.snd_state = SND_STATE_SENDING
                // 如果已经初始化了发送定时器且未停止状态，将再次重启
                if (task_ctx.snd_timer && 'closed' != task_ctx.snd_timer.status) {
        ...
        /**
         * 初始化一个发送task_ctx, 主动create或只有接收到数据时才需要初始化
         * @param {*} cid 
         * @param {*} address 
         * @param {*} port 
         * @param {*} tn 
         ...
         * @returns 
         */
        _snd_task_init(cid, address, port, tn, mtype, seq, swnd, rwnd) {
            var fdtn = this.conn.initoid(cid, tn, address, port)
            // 如果需要，初始化isn
            var task_ctx = this._check_snd_task_ctx(fdtn)
            ...
            // 从 cid 反解出connid信息
            var connid = ConnectId.decode(cid)
            var cf = connid.cf
            ...
            this.conn.newTask(fdtn, SND_NAME, {
                fdtn, tn, cid, isn, address, port, cf, connid,
                rx_rtts: 0,                // 加权平均往返时间
                ...
                dead_link: SND_DEADLINK,   // 重试最大次数后，认为是连接通道异常
        ...
        /**
         * 通过ack中的ack字段，释放snd_buf中的已确认的数据包
         * @param {*} task_ctx 
         * @param {*} una 小于una的所有seq可以释放
         * @param {*} ack 等于ack的seq可以释放
         * @param {*} sack 等于sack集合中所有seq可以释放
         * @param {*} snack 小于una且不等于sack集合中可以释放 TODO
         */
        _release_snd_buf(task_ctx, una, ack, sack, snack) {
            var { snd_buf, nsnd_buf } = task_ctx
            if (!utils.isNull(una)) {
                for (var seq in snd_buf) {
                    seq = Number(seq)
                    if (seq < una) {
                        delete snd_buf[seq]
                        nsnd_buf--
            ...
            if (!utils.isNull(snack) && !utils.isNull(una)) {
                for (var seq in sack) {
                    seq = Number(seq)
                    if (seq < una && snd_buf[seq]) {
                        delete snd_buf[seq]
                        nsnd_buf--
                    }
                }
            }
            task_ctx.nsnd_buf = nsnd_buf
            logger.info('release', task_ctx.nsnd_buf, task_ctx.nsnd_que)
            this._event.emit('release', this)
        }
         ...
         * @returns 
         */
        _snd_parse_fastack(task_ctx, sn, ts) {
            const IKCP_FASTACK_CONSERVE = false
            var { snd_una, snd_nxt, snd_buf } = task_ctx
        ...
        create(fd, swnd, rwnd) {
            ...
            var fdtn = this._snd_task_init(cid, address, port, tn, BEGIN, seq, swnd, rwnd)
            this.conn.newTask(fdtn, CON_NAME, { fd, cf, connid, cid, address, port })
        ...
        kudp_cansend(fdtn) {
            var task_ctx = this._snd_task_ctx(fdtn)
            if (!utils.isNull(task_ctx)) {
                return (task_ctx.snd_buf_wnd - (task_ctx.nsnd_buf + task_ctx.nsnd_que)) * MSS
            }
            return 0
        }
        ...
        kudp_send(fdtn, buf, len, flag) {
            var task_ctx = this._snd_task_ctx(fdtn)
            if (!utils.isNull(task_ctx)) {
                var nbuf = task_ctx.nsnd_que
                var count = len < MSS ? 1 : Math.ceil(len / MSS)
                var i = 0
                // 没有超过最大发送端缓存的限制 snd_buf + snd_que 总和
                while (count > 0 && nbuf < task_ctx.snd_que_siz) {
                    var seg = buf.slice(i, i + MSS)
                    var { seq, pack_ctx } = this._snd_data_seq(task_ctx, seg, seg.length, flag)
                    this._add_snd_que(task_ctx, seq, pack_ctx)
                    nbuf = task_ctx.nsnd_que;
                    i += seg.length
                    count--
                    console.log("send", seg)
                }
                return count > 0 ? -2 : 0
            }
            return -1
         ...
         * @param {*} ackpack 
         */
        __rcv_notify_ack(fdtn, pcf = SND_ACK_PCF_DEFAULT, mtype) {
            var task_ctx = this._snd_task_ctx(fdtn)
            var ack_ctx = this.conn.get_task_ctx(fdtn, ACK_NAME)
            if (!task_ctx || !ack_ctx) {
                this.error(fdtn, ESEMPTY_ACKQ, 'ack list empty')
                return -1
            }
            var { address, port } = task_ctx
            // 遍历
            for (var seq in ack_ctx.list) {
                var { ackpack_ctx, rwnd, una } = ack_ctx.list[seq]
                var { cf, tn, connid, rcv_ts, mtype } = ackpack_ctx
                var ctx = { rcv_ts, rwnd, una }
                var ack_type = ACK_TYPE[mtype]
                var ackpack = new Package(ack_type, pcf, cf, tn, seq, connid, ctx)
                this.kudp_output(address, port, ackpack.buffer)
                ack_ctx.cnt--
                delete ack_ctx.list[seq]
            }
            logger.info("__rcv_notify_ack", fdtn, pcf)
        }
         ...
         * @param {*} mtype 
         */
        __update_snd_rmt_wnd(fdtn, tn, rmt_wnd, mtype, ack_ctx) {
            var task_ctx = this._snd_task_ctx(fdtn)
            if (task_ctx) {
                ...
                if (0 == rmt_wnd) {
                    task_ctx.wnd_timer = task_ctx.wnd_timer || new timer({
                        onend: (args) => {
                            logger.info("timer onend:", ...args)
                            this._snd_0wnd_ask(...args)
                        },
                        onstop: (...args) => { logger.info("timer onstop:", ...args) }
                    }, [fdtn, tn, mtype])
            ...
            logger.info("__update_snd_rmt_wnd", ack_ctx)
            // 清理snd_buf的数据包
            var { una, ack, sack, snack, ts } = ack_ctx
            this._release_snd_buf(task_ctx, una, ack, sack, snack)
            // 计算fastack数据包
            this._snd_parse_fastack(task_ctx, ack, ts)
        }
    ...
    class Recver {
        ...
        _check_rcv_task_ctx(fdtn) {
            return this.conn.get_task_ctx(fdtn, RCV_NAME)
        }
         ...
         */
        _rcv_task_ctx(fdtn) {
            var task_ctx = this._check_rcv_task_ctx(fdtn)
            if (null == task_ctx) {
                console.log(this)
                this.error(fdtn, ERINVALID_FDTN, 'invalid recv fdtn' + fdtn)
            }
            return task_ctx
        }
        ...
        _rcv_task_init(cid, address, port, tn, mtype, seq, rwnd) {
            ...
            if (this.rcv_task[fdtn] && !this.rcv_task[fdtn][RCV_NAME]) {
                ...
                // 从 cid 反解出connid信息
                var connid = ConnectId.decode(cid)
                rcv_ctx.cf = connid.cf
                rcv_ctx.connid = connid
            }
            this.conn.newTask(fdtn, RCV_NAME, rcv_ctx)

            // 初始化发送环境
            var task_ctx = this.sender._check_snd_task_ctx(fdtn)
            if (!task_ctx) {
                this.sender._snd_task_init(cid, address, port, tn, mtype, seq, null, rwnd)
            }
            return fdtn
        }
         ...
         * @param {*} ackpack 
         */
        _rcv_task_dack_push(fdtn, seq, mtype, ackpack_ctx) {
            var task_ctx = this._rcv_task_ctx(fdtn)
            var ack_ctx = this.conn.get_task_ctx(fdtn, ACK_NAME)
            if (task_ctx && !ack_ctx) {
                this.conn.newTask(fdtn, ACK_NAME, {
                    cnt: 0,   // list的大小
                    list: {}, // 待ack的数据包
                })
                ack_ctx = this.conn.get_task_ctx(fdtn, ACK_NAME)
            }
            // 将需要ack的数据包写入 ack 集合中，方便发送器在下次遍历发送到对端
            var rwnd = this._rcv_task_rwnd(fdtn)  // 本端接收窗口
            ack_ctx.list[seq] = { ackpack_ctx, rwnd, una: task_ctx.rcv_nxt }
            ack_ctx.cnt++
            // 计算una, sack, nack
            this.sender.__rcv_notify_ack(fdtn, SND_ACK_PCF_DEFAULT, mtype)
        }
         ...
         * @param {*} ackpack_ctx 
         */
        _rcv_task_cack_push(fdtn, seq, mtype, ackpack_ctx) {
            var task_ctx = this._rcv_task_ctx(fdtn)
            var ack_ctx = this.conn.get_task_ctx(fdtn, ACK_NAME)
            if (task_ctx && !ack_ctx) {
                this.conn.newTask(fdtn, ACK_NAME, {
         ...
         * @param {*} ctx 
         */
        _rcv_parse_data(fdtn, seq, mtype, newseg) {
            var task_ctx = this._rcv_task_ctx(fdtn)
            var { rcv_buf, nrcv_buf, rcv_que, nrcv_que, rcv_nxt, rcv_wnd } = task_ctx
            var repeat = 0
            // 把数据ctx 中数据包放到 rcv_buf 正确的位置
            if (rcv_nxt == -1) {
                rcv_buf[seq] = newseg
                nrcv_buf++
            } else {
                 ...
                 *         rcv_nxt                 rcv_max
                 * 
                 * rcv_max = rcv_nxt + rcv_wnd
                 * 
                 *   + 已经接收序号段    | 下一个待接收的数据包序号
                 *   - 待接收的序号段    ^ 最大的接收数据包序号
                 *   ~ 超过接收窗口的序号
                 */
                if (seq < rcv_nxt || seq > rcv_nxt + rcv_wnd) {
                    logger.warn("_rcv_parse_data", fdtn, seq, mtype, newseg)
                    return -1
                }
                rcv_buf[seq] = newseg
                nrcv_buf++
            ...
            for (var seq in rcv_buf) {
                if (seq == rcv_nxt && nrcv_que < rcv_wnd) {
                    rcv_que[seq] = rcv_buf[seq]
                    nrcv_que++
                    delete rcv_buf[seq]
                    nrcv_buf--
                    task_ctx.rcv_nxt++
        ...
        kudp_input(address, port, buffer) {
            ...
            if (!kudpack) {
                this.error(fdtn, ERINVALID_PACK, 'invalid recv package')
                return -1
            }
            ...
            if (version != VERSION ||                          // 不合法的版本号
                ...
                this.error(fdtn, ERINVALID_PACK2,
                    'invalid recv package, check error' + header.toString())
                return -2
            }
            // 数据包体中必要的信息
            ...
            if (DEBUG) {
                logger.info(kudpack.toHex(), body, ctx)
                logger.info(header.toHex(), HeaderType[header.Type])
                logger.info(HeaderType[header.Type])
            }
            ...
            switch (mtype) {
                ...
                case CLOSE: // 关闭连接fd，task
                    this.sender.__rcv_notify_ctrl(fdtn, tn, pcf, mtype)
                    break;
                case WSANS: // 窗口的大小回复通知, 更新 rmt_wnd
                case CTACK: // 控制流的数据包ACK, 更新 rmt_wnd
                case DTACK: // 更新rtt rmt_wnd
                    var ack_ctx = { pcf, cf, tn, connid, mtype, ts, ack: seq, ...ctx }
                    this.sender.__update_snd_rmt_wnd(fdtn, tn, rmt_wnd, mtype, ack_ctx)
                    break;

                /* 接收到以下类型的数据包，需要生成对应数据包作为"响应" */
                case WSASK: // 计算wnd, 生成WSANS返回
                case PINGS: // 计算wnd, 生成CTACK返回
                    var ackpack_ctx = { cf, tn, connid, rcv_ts, rmt_wnd, mtype, ts }
                    this._rcv_task_cack_push(fdtn, seq, mtype, ackpack_ctx)
                    break;
                ...
                case BDODO: // 计算ACK
                    var ackpack_ctx = { pcf, cf, tn, connid, rcv_ts, rmt_wnd, mtype, ts }
                    this._rcv_task_dack_push(fdtn, seq, mtype, ackpack_ctx)
                    // 写入rcv_buf
                    this._rcv_parse_data(fdtn, seq, mtype, ctx)
                    break
                default:
                    this.error(fdtn, ERINVALID_MTYPE, 'invalid package type')
                    break;
            }
        }
        ...
        kudp_recv(fdtn) {
            var task_ctx = this._rcv_task_ctx(fdtn)
            if (task_ctx) {
                var { rcv_buf, nrcv_buf, rcv_que, nrcv_que } = task_ctx
                var len = 0
                for (var seq in rcv_que) {
                    var segment = rcv_que[seq]
                    var payload = segment.payload
                    var len = payload.length
                    this.recv && this.recv(fdtn, payload, len)
                    nrcv_que--
                    len += payload.length
                    delete rcv_que[seq]
                }
    ...
    class kudp extends UdpBase {
        ...
        emit(type, listener) {
            this._event.emit(type, listener)
         ...
         */
        sendbuf_size(fdtn) {
            return this.sender.kudp_cansend(fdtn)
        }
         ...
         */
        recv(callback) {
            return this.recver.recv = callback
        }

        /**
         * 从一个kudp连接返回异常信息, 回调的方式
         * @param {int} fdtn 由`create`创建出来的`fd task number` 0 表示连接级别
