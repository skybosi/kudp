...
(function (g, f) {
    ...
    const KUDP_RCVBUF = 1024 // 4194304 // 4 * 124 * 1024 kb
    ...
    class Recver {
        ...
        checkRcvBuf(node, deltaData) {
            ...
            if (len + offset <= max) {
                ...
                node['P']++;
                if (this.isOver(node)) {
                    LOG.info("checkRcvBuf", node['BF']._woffset)
                    this.onUp(node['seq'], DONED, node['P'], node['peer'], node['BF'].slice(0, node['BF']._woffset));
                }
            } else if (len + offset >= max) {
                this.onUp(node['seq'], node['T'], node['P'], node['peer'], node['BF']);
                node['BF'].flush();
                node['BF'].write(deltaData);
                node['P']++;
            } else {
    ...
    class Sender {
        ...
        retry(seq, mtype, ip, port, pack, ctx) {
            LOG.warn('retry: ', seq, mtype, ip, port)
