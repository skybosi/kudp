...
(function (root, factory) {
  ...
  class Sender {
    ...
    defaultOptions = {
      ...
      repeat: 100,           // 发送超时最大次数，超过时将报错，回调onErrs
    }
    ...
    free(task_seq, mtype, peerInfo) {
      ...
      if (mtype > AMULTI) {
        ...
        if (ATask && ATaskQ && !ATaskQ.empty()) {
          ...
          console.info("free ack seq ok task peer:", task, seq, task_seq);
        }
     ...
     * @param {*} payload 
     */
    send(fd, ip, port, type, payload) {
      let body = null;
      if (type < ABROAD) {
        let PACK_SIZE = utils.IsLanIP(ip) ? WAN_PACK_SIZE : LAN_PACK_SIZE;
        body = this.encode(fd, type, payload, PACK_SIZE);  // 编码数据包
      } else {
        body = this.encodeAck(type, payload, ACK_PAYLOAD);          // 编码数据包ACK
      }
      let { task_seq, size, mtype, pack } = body;
      // 发送缓冲区溢出异常
      if (0 > size) return { size, task_seq, mtype };
      this.onSend(mtype, ip, port, pack.buffer);      
      this.addSeqTimer(task_seq, mtype, ip, port, pack, fd);
      return { size, task_seq, mtype };
    }
    ...
    encode(fd, mtype, payload, max_size) {
      ...
      switch (mtype) {
        case BEGIN:
          task_seq = this.seqer.malloc();
          fstat['isn'] = task_seq;
          break;
        case BDD: case BROAD: case MULTI:
          task_seq = this.seqer.malloc(1);
          fstat['isn'] = task_seq;
          break;
        case DOING: case DONED:
          ...
          task_seq = this.seqer.next(fstat['isn']);
          break;
        default:
          throw Errors(EHEADERTYPE, "invalid type", type);
      }
      let pack = new kupack(mtype, 0, 1, task_seq, buff);
      return { task_seq, size, mtype, pack }
    }
     ...
     * @param {*} payload 
     */
    encodeAck(mtype, task_seq, payload) {
      let buff = this._encoding(payload);
      let pack = new kupack(mtype, 0, 1, task_seq, buff);
      return { task_seq, size: buff.length, mtype: pack.header.Type(), pack }
    }
    ...
    addSeqTimer(task_seq, mtype, ip, port, pack, fd) {
      let { task, seq, task_info } = this.taskSeq(task_seq)
      let isn = task_info['isn']
      console.info("send data addSeqTimer type task peer", mtype, task, seq, task_seq)
      switch (mtype) {
        case BEGIN: case BDD:
          let cap = (mtype === BEGIN) ? KUDP_SNDBUF : 1;
          let data = { mtype, pack, task_seq, task, isn, fd, ip, port, stime: +new Date }
          let ATask = this.queue.insert(task, seq, cap, data, this.retry.bind(this));
          if (ATask) {
      ...
      let ATaskQ = ATask['Q']
      if (ATask && ATaskQ) {
        let data = { mtype, pack, task_seq, task, seq, fd, ip, port, stime: +new Date }
        this.queue.push(task, seq, data);  // TODO 是否存在 ip, port 错乱
        if (mtype === DONED) {
          let ATaskTimer = ATask['timer']
          ATaskTimer && ATaskTimer.setArgs({ done: 1 });
        }
