...
(function (g, f) {
  ...
  const EHEADERTYPE = errors.EHEADERTYPE
  const ENOTALLOCISN = errors.ENOTALLOCISN
  ...
  class SeqManage {
    ...
    /**
     * 获取当前task的最大待使用的seq
     * @param {*} tn task number
     * @returns
     */
    cursor(tn) {
      return this.seqs[tn]['cursor']
    }
  ...
  // 数据包类型
  const BROAD = 0x2   // 发送广播数据包
  const MULTI = 0x3   // 发送多播数据包
  const WSASK = 0x4   // 窗口的大小询问
  const WSANS = 0x5   // 窗口的大小回复通知
  const PINGS = 0x6   // 验证与维持对端是否仍然存活
  const RESET = 0x7   // 重置发送过程，可以是task级别，也可以是connection级别
  const CLOSE = 0x8   // 关闭连接，最终释放所有的资源，正在传输的将继续，但不再处理其他的新任务
  const CTACK = 0x9   // 控制流的数据包ACK
  ...
  const KUDP_MAX_FIXHEAD = 10  // 固定头部大小
  const KUDP_MAX_VARHEAD = 10  // 固定头部大小
  ...
  class ConnectId {
    ...
    encode() {
      return CID_PREFIX + (this.high ? ":" + this.high.toString(16).replace(/(.{2})/g, '$1').trim() : "") +
        (this.low ? ":" + this.low.toString(16).replace(/(.{2})/g, '$1').trim() : "")
    }
    static decode(cid) {
      if ("string" != typeof cid || "" == cid) {
        return null
      }
      var cidlist = cid.match(new RegExp(CID_PREFIX + ":?([0-9a-fA-F]*):?([0-9a-fA-F]*)"))
      var left = cidlist[1], right = cidlist[2]
      var cf = 0, low = 0, high = 0
      if ("" == right && "" == left) {        // cid
        cf = 0
      } else if ("" != right && "" != left) { // cid:xxx:yyy
        cf = 3
        high = Number("0x" + left)
        low = Number("0x" + right)
      } else if ("" == right && "" != left) { // cid:yyy
        low = Number("0x" + left)
        cf = (low <= MAXINT8) ? 1 : 2
      } else {
        cf = 0
      }
      return new ConnectId(cf, low, high)
    }
    toString() {
  ...
  class Header {
    ...
    get Cid() { return this.connid ? this.connid.cid : null; }
    ...
    toHex() {
      if (utils.isNull(this.connid) || this.connid.length == 0) {
  ...
  class Body {
    ...
    static encodeBody(type, pcf, tnseq, ctx) {
      ...
      switch (type) {
          ...
          switch (pcf) {
            ...
            case 1:
              ...
              for (let k in ctx.kind) {
                ...
                } else if ("number" == typeof v.value) {
                  len = 4
                } else if ("string" == typeof v.value) {
                  len = v.value ? v.value.length : 0
                } else if ("string" == typeof v.value) {
        ...
        case CTACK: // "CTACK"
          /**
           * header-0: PCF = 0b00 (default) 控制流的数据包ACK
           *     8      7      6      5      4      3      2      1      0
           *     +------+------+------+------+------+------+------+------+
           *     |   0      0      0  |   0     1       0      0      1  | Type: 9
           *     +------+------+------+------+------+------+------+------+
           *     |   x      x      x      x  |   0      0  |   ?      ?  | Flag = 0 | 1 | 2 | 3
           *     +------+------+------+------+------+------+------+------+
           *     |              connection id(optional)                  | cid
        ...
        case BEGIN: // "BEGIN"
          ...
          switch (pcf) {
            case 0:
              bbuf = new Buffer(MSS + KUDP_MAX_VARHEAD)
              bbuf.writeUInt32BE(ts)                // ts
        ...
        case DTACK: // "DTACK"
           ...
           *     |              connection id(optional)                  | cid
           *     +------+------+------+------+------+------+------+------+
           * bbuf-2: PCF = 0b10 -> ack + una + sack， 必须是同一个连接，建议是同一个task，但允许不同task，表示已被确认的
           *     0                             4       5                     8
           *     +-----------------------------+-------+---------------------+
           *     |         echo ts(4)          | tn(1) |       seq(3)        |  tn + seq = ack
           *     +-----------------------------+-------+---------------------+
           ...
           *
           * header-3: PCF = 0b11 -> ack + snack， 必须是同一个连接
           *     8      7      6      5      4      3      2      1      0
           *     +------+------+------+------+------+------+------+------+
           *     |   0      0      0  |   1     0       1      0      0  | Type: 20
           *     +------+------+------+------+------+------+------+------+
           ...
           *     |              connection id(optional)                  | cid
           *     +------+------+------+------+------+------+------+------+
           * bbuf-3: PCF = 0b11 -> ack + snack， 必须是同一个连接，建议是同一个task，但允许不同task，表示未被确认的
           *     0                             4       5                     8
           *     +-----------------------------+-------+---------------------+
           *     |         echo ts(4)          | tn(1) |       seq(3)        |  tn + seq = ack
           *     +-----------------------------+-------+---------------------+
          ...
          switch (pcf) {
            ...
            case 2:
              ...
              bbuf.writeUInt32BE(ctx.rwnd)          // rwnd
              bbuf.writeUInt32BE(ctx.una)           // una
              for (let asack in ctx.sack) {
                bbuf.writeUInt32BE(asack)           // each tn + seq
              }
              break;
            case 3:
              bbuf = new Buffer(MSS)
              ...
              bbuf.writeUInt16BE(delta)             // delta
              bbuf.writeUInt32BE(ctx.rwnd)          // rwnd
              for (let asnack in ctx.snack) {
                bbuf.writeUInt32BE(asnack)          // each tn + seq
              }
              break;
            default:
              break;
    ...
    static decodeBody(type, pcf, buffer, offset) {
      ...
      switch (type) {
        case BROAD: // "BROAD"
          switch (pcf) {
            case 0:
              ret.tn = buffer.readUInt8(0 + offset)             // [5, 5) tn
              break;
            case 1:
              ret.tn = buffer.readUInt8(0 + offset)             // [5, 5) tn
              ret.ctx = {}
              ret.ctx.kind = []
              for (var i = offset + 1; i < buflen;) {
        ...
        case WSANS: // "WSANS"
          ...
          switch (pcf) {
            case 0: case 2:
              ret.ts = buffer.readUInt32BE(0 + offset)          // [0, 4) ts
              ret.tn = buffer.readUInt8(4 + offset)             // [4, 5) tn
              ret.ctx.delta = buffer.readUInt16BE(5 + offset)   // [5, 7)  delta
              ret.ctx.rwnd = buffer.readUInt32BE(7 + offset)    // [7, 11) rwnd
              break;
            case 1: case 3:
              ret.ts = buffer.readUInt32BE(0 + offset)          // [0, 4) ts
              ret.tn = buffer.readUInt8(4 + offset)             // [4, 5) tn
        ...
        case CTACK: // "CTACK"
          ...
          switch (pcf) {
            case 0:
              ret.ts = buffer.readUInt32BE(0 + offset)          // [0, 4) ts
              ret.ctx.rwnd = buffer.readUInt32BE(4 + offset)    // [4, 8) rwnd
              break;
            default:
              break;
          }
        ...
        case BEGIN: // "BEGIN"
          ...
          switch (pcf) {
            case 0:
              ...
              ret.ctx.payload = buffer.slice(8 + offset)        // [8, )  payload
              break;
            default:
              break;
          }
        ...
        case DTACK: // "DTACK"
          ...
          switch (pcf) {
            case 0:
              ...
              var tnseq = buffer.readUInt32BE(4 + offset)       // [4, 8) tn + seq
              var { tn, seq } = SeqManage.decode(tnseq)
              ret.tn = tn, ret.seq = seq, ret.ack = seq
            ...
            case 2:
              ...
              for (var i = 18 + offset; i < buflen; i += 4) {
                let asack = buffer.readUInt32BE(i);             // each tn + seq
                var { tn, seq } = SeqManage.decode(asack)
                ret.ctx.sack.push({ ack: seq, tn })
              }
              break;
            case 3:
              ret.ts = buffer.readUInt32BE(0 + offset)          // [0, 4) ts
              ...
              for (var i = 14 + offset; i < buflen; i += 4) {
                let asnack = buffer.readUInt32BE(i);             // each tn + seq
                var { tn, seq } = SeqManage.decode(asnack)
                ret.ctx.snack.push({ nack: seq, tn })
              }
              break;
            default:
              break;
  ...
  exports.CTACK = CTACK; // 控制流的数据包ACK
  ...
  exports.HeaderType = HeaderType    // 类型
  exports.rHeaderType = rHeaderType  // 反类型
  ...
  exports.MSS = MSS
  exports.SEP = SEP
  exports.MIN_TASK_NUM = MIN_TASK_NUM
