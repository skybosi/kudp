...
#### Flag
- CF(1): connection id flag
  - 0: 不开启 connection id
  - 1: 开启 connection id， 64bit = 8Byte
...
#### 连接ID (connection id)
- CF = 1 时，用于保持连接的标识，8Byte
- CF = 0 时，connection id 禁止携带 connection id，局域网建议设置为0
...
header-2: PCF = 10 -> ack + una + sack， 必须是同一个连接，建议是同一个task，但允许不同task
    ...
    |              connection id(optional)                  | cid
    +------+------+------+------+------+------+------+------+
body-2: PCF = 10 -> ack + una + sack， 必须是同一个连接，建议是同一个task，但允许不同task
    0                             4       5                     8
    +-----------------------------+-------+---------------------+
    |         echo ts(4)          | tn(1) |       seq(3)        |  tn + seq = ack
    +-----------------------------+-------+---------------------+
...
header-3: PCF = 11 -> ack + sack， 必须是同一个连接
    ...
    |              connection id(optional)                  | cid
    +------+------+------+------+------+------+------+------+
body-3: PCF = 11 -> ack + sack， 必须是同一个连接，建议是同一个task，但允许不同task
    0                             4       5                     8
    +-----------------------------+-------+---------------------+
    |         echo ts(4)          | tn(1) |       seq(3)        |  tn + seq = ack
    +-----------------------------+-------+---------------------+
